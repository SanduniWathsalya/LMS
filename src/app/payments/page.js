...
export default function PaymentPage() {
  ...
  const subjectList = ['Maths', 'Science', 'English', 'IT'];
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    const data = {
      ...
      paymentType,
      selectedSubjects,
      semester: paymentType === "semester" || paymentType === "term" ? semester : '',
      paymentMonth,
      paymentDate,
    };
        ...
        <div className="flex items-end justify-between h-28 bg-blue-950 border-b border-gray-200 px-4">
          ...
          <div className="flex items-end mb-2 space-x-4">
            ...
            <a href="#" className="relative flex flex-col items-center text-white hover:text-gray-300 group">
              ...
              <span className="absolute top-full mt-1 text-xs text-white bg-gray-800 px-1 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200">Notifications</span>
            </a>
            ...
            <a href="#" className="relative flex flex-col items-center text-white hover:text-gray-300 group">
              ...
              <span className="absolute top-full mt-1 text-xs text-white bg-gray-800 px-1 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200">Logout</span>
            </a>
          </div>
        </div>
          ...
          <div className="w-1/5 space-y-2">
            {grades.map((grade) => (
              <div
                ...
                className={`text-xl text-center py-3 rounded-md cursor-pointer shadow-md hover:opacity-90 transition-colors duration-300 ${selectedGrade === grade ? 'bg-white text-blue-950 border border-blue-950' : 'bg-blue-950 text-white'}`}
              >
            ...
            {!selectedGrade ? (
              <div className="p-6 border rounded shadow-md bg-white mb-10">
                ...
                <form className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={studentId} onChange={(e) => setStudentId(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-600" placeholder="Enter Student ID" />
                  </div>

                  {/* Payment Type */}
                  <div>
                    <label className="block text-sm font-medium text-black">Payment Type</label>
                    <select value={paymentType} onChange={(e) => setPaymentType(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-600">
                      <option value="">Select Payment Type</option>
                      <option value="subject">Subject-wise</option>
                      <option value="semester">Semester-wise</option>
                      <option value="term">Term-wise</option> {/* ✅ NEW */}
                    </select>
                  </div>
                  ...
                  {paymentType === "subject" && (
                    <>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2 text-black">
                        {subjectList.map((subject) => (
                          <label key={subject} className="flex items-center gap-2">
                            <input type="checkbox" value={subject} onChange={(e) => handleSubjectSelect(e)} />
                            {subject}
                          </label>
                        ))}
                      </div>
                      <div>
                        ...
                        <select value={paymentMonth} onChange={(e) => setPaymentMonth(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-600">
                      ...
                      <div>
                        ...
                        <input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-600" />
                      </div>
                    </>
                  )}
                  ...
                  {paymentType === "semester" && (
                    <>
                      <div>
                        ...
                        <select value={semester} onChange={(e) => setSemester(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-600">
                  ...
                  {paymentType === "term" && (
                    <>
                      <div>
                        <label className="block text-sm font-medium text-black">Select Term</label>
                        <select value={semester} onChange={(e) => setSemester(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-gray-600">
                  ...
                  <div>
                    <button type="submit" onClick={handlePaymentSubmit} className="bg-gradient-to-r from-blue-600 to-blue-800 text-white px-4 py-2 rounded hover:from-blue-700 hover:to-blue-900 shadow-md transition transform hover:-translate-y-1">
                      Submit Payment
                    </button>
                  </div>
                </form>
              ...
              <>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setSelectedGrade(null)} className="relative group bg-gradient-to-r from-blue-600 to-blue-800 text-white px-4 py-2 rounded-full hover:from-blue-700 hover:to-blue-900 text-sm flex items-center gap-2 shadow-md transition transform hover:-translate-y-1">
                ...
                <ul className="space-y-2 text-black">
                  {students.map((student, index) => (
                    <li key={index} className="flex justify-between items-center border p-3 rounded shadow-sm hover:bg-gray-50 transition">
                      <div className="flex gap-4">
                        <span><strong>{student.sid}</strong></span>
                        <span>{student.name}</span>
                      </div>
                      <Link href="/student_payment">
                        <button className="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 text-sm">View</button>
                      </Link>
                    </li>
                  ))}
                </ul>
